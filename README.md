...
## Backtesting (Work in Progress)

The next development phase of this project focuses on validating the effectiveness of the generated investment signals through historical backtesting.

The objective is to determine whether sentiment-driven trading signals generated by FinBERT have predictive value over future market movements.

The planned backtesting framework will:

- Retrieve historical financial news for each supported asset.
- Generate historical sentiment scores using the same FinBERT pipeline used in live inference.
- Replay the signal generation process on historical data.
- Compare generated Buy/Sell/Hold signals against actual future price movements.
- Evaluate the strategy using statistical and trading performance metrics.
...
The goal is not only to determine whether the system is profitable, but also to identify its strengths, weaknesses, and the market conditions under which sentiment-based signals are most reliable.
