...
│   ├── btc_backtesting.py        # Runs the Gold (GC=F) backtest -> backtesting/data/gold_backtest_1y.csv
│   ├── gold_backtesting.py         # Runs the Bitcoin (BTC-USD) backtest -> backtesting/data/btc_backtest_1y.csv
...
- **Per-asset runner scripts** (`run_gold_backtest.py`, `run_btc_backtest.py`) each pull a configurable historical window (currently 1 year) for their asset independently.
...
- **Output**: dated CSVs under `backtesting/data/` (e.g. `gold_backtest_1y.csv`, `btc_backtest_1y.csv`), currently analyzed externally in Google Colab.

**Still to come (evaluation stage):**
